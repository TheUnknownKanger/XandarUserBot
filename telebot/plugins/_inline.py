...
MESAG = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "`XandarBot PM security! Please wait for me to approve you. 😊"
)
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "XandarBot User"
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("`Userbot"):
            ...
            result = builder.article(
                title="Stats",
                text=f"**XandarBot Stats For [{DEFAULTUSER}](tg://user?id={myid})**\n\n__Bot is functioning normally, master!__\n\n(c) @xandar0202",
                buttons=[
                    [custom.Button.inline("Stats", data="statcheck")],
                    [Button.url("Repo", "https://github.com/lucifeermorningstar/XandarUserBot")],
                    [
                        Button.url(
                            "Deploy Now!",
                            "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Flucifeermoringstar%2FXandarUserBot&template=https%3A%2F%2Fgithub.com%2Flucifeermorningstar%2FXandarUserBot",
                        )
                    ],
                ],
            )
            ...
            result = builder.article(
                "Source Code",
                text="**Welcome to XandarBot**\n\n`Click below buttons for more`",
                buttons=[
                    [custom.Button.url("Creator👨‍🦱", "https://t.me/xandar0202")],
                    [
                        custom.Button.url(
                            "👨‍💻Source Code‍💻", "https://github.com/lucifeermorningstar/XandarUserBot"
                        ),
                        custom.Button.url(
                            "Deploy 🌀",
