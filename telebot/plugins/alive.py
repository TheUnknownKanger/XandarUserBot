...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "@xandar0202"
...
async def amireallyalive(alive):
    ...
    if ALV_PIC:
        tele = f"**Welcome To XandarBot **\n\n"
        tele += f"`{CUSTOM_ALIVE}`\n\n"
        tele += (
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
        )
        tele += f"{telemoji} **XandarBot Version**: `{telever}`\n"
        tele += f"{telemoji} **More Info**: @xandar0202\n"
        tele += f"{telemoji} **Sudo** : `{sudo}`\n"
        tele += f"{telemoji} **XandarBot Uptime**: `{uptime}`\n"
        tele += f"{telemoji} **Database Status**: `All OK 👌!`\n"
        tele += (
            f"{telemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
        )
        tele += "    [✨ GitHub Repository ✨](https://github.com/lucifeermorningstar/XandarUserBot)"
        ...
        await alive.delete()
        return
    req = requests.get("https://telegra.ph/file/ac867b92a49becbc321c6.jpg")
    ...
    with BytesIO() as sticker:
        ...
        await borg.send_message(
            alive.chat_id,
            f"**Welcome To XandarBot **\n\n"
            f"`{CUSTOM_ALIVE}`\n\n"
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
            f"{telemoji} **XandarBot Version**: `{telever}`\n"
            f"{telemoji} **More Info**: @xandar0202\n"
            f"{telemoji} **Sudo** : `{sudo}`\n"
            f"{telemoji} **XandarBot Uptime**: `{uptime}`\n"
            f"{telemoji} **Database Status**: `All OK 👌!`\n"
            f"{telemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
            "    [✨ GitHub Repository ✨](https://github.com/lucifeermorningstar/XandarUserBot)",
            link_preview=False,
        )
