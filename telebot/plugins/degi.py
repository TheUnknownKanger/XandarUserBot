...
@telebot.on(admin_cmd(pattern="degi$"))
@telebot.on(sudo_cmd(pattern="degi$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@telebot.on(admin_cmd(pattern=f"nehi$", outgoing=True))
@telebot.on(sudo_cmd(pattern=f"nehi$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
